...
|------------- | ------------- | -------------|
| [**sandboxDataImport**](SandboxApi.md#sandboxdataimportoperation) | **POST** /obp/v2.1.0/sandbox/data-import | Create sandbox |
...
import type { SandboxDataImportOperationRequest } from 'obp-typescript';
...
async function example() {
  ...
  const body = {
    // SandboxDataImportRequest | Request body
  ...
  } satisfies SandboxDataImportOperationRequest;

  try {
    const data = await api.sandboxDataImport(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
