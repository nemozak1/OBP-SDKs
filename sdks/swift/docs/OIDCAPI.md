...
// Get JSON Web Key (JWK) URIs
OIDCAPI.getOAuth2ServerJWKsURIs() { (response, error) in
...
// Get Connector Status (Loopback)
OIDCAPI.getObpConnectorLoopback() { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Get OIDC Client
OIDCAPI.getOidcClient(clientid: clientid) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
...
// Verify OIDC Client
OIDCAPI.verifyOidcClient(verifyOidcClientRequest: verifyOidcClientRequest) { (response, error) in
    guard error == nil else {
        print(error)
        return
    }
