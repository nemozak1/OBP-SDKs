...
/// struct for typed errors of method [`get_aggregate_metrics`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetAggregateMetricsError {
    Status500(),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`get_metrics_top_consumers`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetMetricsTopConsumersError {
    Status500(),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_aggregate_metrics(configuration: &configuration::Configuration, ) -> Result<models::GetAggregateMetrics200Response, Error<GetAggregateMetricsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetAggregateMetrics200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetAggregateMetrics200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetAggregateMetricsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn get_metrics_top_consumers(configuration: &configuration::Configuration, ) -> Result<models::GetMetricsTopConsumers200Response, Error<GetMetricsTopConsumersError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetMetricsTopConsumers200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetMetricsTopConsumers200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetMetricsTopConsumersError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
