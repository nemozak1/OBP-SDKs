...
/// struct for typed errors of method [`get_api_glossary`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetApiGlossaryError {
    Status500(),
...
pub async fn get_api_glossary(configuration: &configuration::Configuration, ) -> Result<models::GetApiGlossary200Response, Error<GetApiGlossaryError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetApiGlossary200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetApiGlossary200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetApiGlossaryError> = serde_json::from_str(&content).ok();
