...
/// struct for typed errors of method [`sandbox_data_import`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SandboxDataImportError {
    Status500(),
...
pub async fn sandbox_data_import(configuration: &configuration::Configuration, sandbox_data_import_request: models::SandboxDataImportRequest) -> Result<models::UpdateTransactionNarrative200Response, Error<SandboxDataImportError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_sandbox_data_import_request = sandbox_data_import_request;
    ...
    req_builder = req_builder.json(&p_body_sandbox_data_import_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<SandboxDataImportError> = serde_json::from_str(&content).ok();
