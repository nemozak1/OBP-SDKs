# SaveHistoricalTransaction200Response

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**description** | Option<**String**> |  | [optional]
**to** | Option<[**models::SaveHistoricalTransactionRequestTo**](SaveHistoricalTransactionRequestTo.md)> |  | [optional]
**transaction_id** | Option<**String**> |  | [optional]
**transaction_request_type** | Option<**String**> |  | [optional]
**completed** | Option<**String**> |  | [optional]
**charge_policy** | Option<**String**> |  | [optional]
**from** | Option<[**models::SaveHistoricalTransactionRequestTo**](SaveHistoricalTransactionRequestTo.md)> |  | [optional]
**value** | Option<[**models::GetMyCustomersAtBank200ResponseCustomersInnerCreditLimit**](GetMyCustomersAtBank200ResponseCustomersInnerCreditLimit.md)> |  | [optional]
**posted** | Option<**String**> |  | [optional]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


